...
const Eresources = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/eresources");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `https://drc-server.onrender.com/deleteeresources/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/eresources_online_add`,
          formData,
          {
            headers: {
  ...
  const handleSubmit1 = async (e) => {
    ...
    const response = await fetch(
      "https://drc-server.onrender.com/eresources_online_add_link",
      {
        method: "POST",
        headers: {
                      ...
                      <li className="mt-2 list-none" key={_id}>
                        <div className="flex flex-col mr-10 ">
                          <h1 className="mt-5">
                            <FontAwesomeIcon
                              icon={faArrowRight}
                              className="ml-20 font-medium text-justify text-base md:text-lg  md:text-left text-blue-400"
                            />
                            <a
                              href={path}
                              target="_blank"
                              className="ml-4 font-medium text-justify text-base md:text-lg  md:text-left text-blue-400 hover:pl-3"
                            >
                              {" "}
                              {link}{" "}
                            </a>
                            <p className=" ml-28 mt-2 leading-14 font-medium text-justify text-base md:text-lg">
                              {title}
                            </p>
                          </h1>
                        </div>
