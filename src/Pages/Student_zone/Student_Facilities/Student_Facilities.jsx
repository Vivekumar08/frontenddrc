...
const Facilities = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Student_Facilities");
  ...
  const handleSubmit_img = async (id,file) => {
    try {
      if (file) {
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Student_Facilities_img_upload/${id}`,
          { file: file },
          {
            headers: {
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_Student_Facilities_fac/${id}`, {
      method: "POST",
    });
    await response.json();
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `https://drc-server.onrender.com/delete_img_Student_Facilities_fac/${id}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`https://drc-server.onrender.com/Student_Facilities_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
