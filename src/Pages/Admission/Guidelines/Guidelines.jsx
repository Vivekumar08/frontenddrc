...
import Sidebar from "../../../Components/Sidebar/Admission_side.";
import Banner from "../../../Components/Banners/Guidelines";
import AuthContext from "../../../Context/AuthProvider";
import Res_fac_data from "./Guidelines_dat";
...
const Guidelines = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Guidelines");
  ...
  const handleSubmit_img = async (id,file) => {
    try {
      if (file) {
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Guidelines_img_upload/${id}`,
          { file: file },
          {
            headers: {
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_Guidelines_fac/${id}`, {
      method: "POST",
    });
    await response.json();
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `https://drc-server.onrender.com/delete_img_Guidelines_fac/${id}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`https://drc-server.onrender.com/Guidelines_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
