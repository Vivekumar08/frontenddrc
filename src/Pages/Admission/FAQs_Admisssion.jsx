...
const FAQs_Admission = () => {
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(
      `https://drc-server.onrender.com/deleteAdmission_FAQs/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit1 = async (e) => {
    ...
    const response = await fetch(
      "https://drc-server.onrender.com/Admission_FAQs_add_link",
      {
        method: "POST",
        headers: {
        ...
        <div className="name">
          <div className="flex flex-row justify-center">
            <p className="  text-[#fff] text-3xl md:text-4xl lg:text-6xl shadow-lg  mt-12 font-bold  p-5 flex justify-center w-full rounded-md  ">
              FAQ<p className="lowercase mr-4">s </p>for Admissions
            </p>
          </div>
