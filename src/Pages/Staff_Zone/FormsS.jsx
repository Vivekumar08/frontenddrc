...
const Forms = () => {
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(
      `https://drc-server.onrender.com/delete_StaffZone_forms/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/StaffZone_forms_add`,
          formData,
          {
            headers: {
  ...
  const handleSubmit1 = async (e) => {
    ...
    const response = await fetch(
      "https://drc-server.onrender.com/StaffZone_forms_add_link",
      {
        method: "POST",
        headers: {
