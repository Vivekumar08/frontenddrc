...
const Roster = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch(
      "https://drc-server.onrender.com/Staff_Roster"
    );
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(
      `https://drc-server.onrender.com/delete_Staff_Roster/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(
          `https://drc-server.onrender.com/Staff_Roster_add`,
          formData,
          {
            headers: {
                ...
                return (
                  <>
                    <div class="card2 mb-10" key={_id}>
                      <span className="  font-bold text-xl ml-2 w-[200px] text-justify">
                        {link}
                      </span>
