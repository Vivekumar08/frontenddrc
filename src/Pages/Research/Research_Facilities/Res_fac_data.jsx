...
const Res_fac_data = (props) => {
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `https://drc-server.onrender.com/delete_research_fac/${id}`,
      {
        method: "POST",
      }
    );
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    const response = await axios.post(
      `https://drc-server.onrender.com/delete_img_research_fac/${id}`,
  ...
  const handleSubmit = async (id) => {
    ...
    try {
      if (file) {
        ...
        setErrMsg("");
        const response = await axios.post(
          `https://drc-server.onrender.com/research_img_upload/${id}`,
          { file: file },
          {
            headers: {
