...
const teacher = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Teacher_In_Charge");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `https://drc-server.onrender.com/delete_Teacher_In_Charge/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        setErrMsg("");
        await axios.post(`https://drc-server.onrender.com/Teacher_In_Charge_add`, {
          Tic1: tic1,
          Tic2: tic2,
          link: link,
          title: caption,
          ...
          <div className="main flex-col whitespace-none">
            <table className=" w-96 h-48 ml-3 md:ml-26 md:w-[500px] md:h-[160px] lg:table-fixed lg:ml-32 lg:w-[800px] lg:h-[180px] mt-1 ">
              <tr className="h-12 md:h-14 lg:h-20 text-lg">
                ...
                <th className="text-sm md:text-sm lg:text-lg w-[20%]">Department</th>
