...
function Events() {
  ...
  const fetchdata = async () => {
    const response = await fetch(
      "https://drc-server.onrender.com/Philo_Events"
    );
  ...
  const del = async (id) => {
    // console.log(id);
    const response = await fetch(
      `https://drc-server.onrender.com/delete_Philo_Events/${id}`,
      {
        method: "POST",
      }
    );
  ...
  const handleSubmit_file = async (id, pdf) => {
    try {
      if (pdf) {
        await axios.post(
          `https://drc-server.onrender.com/Philo_Events_file_add/${id}`,
          {
            file: pdf,
          },
          {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (caption.trim() !== "") {
        ...
        await axios.post(
          `https://drc-server.onrender.com/Philo_Events_add`,
