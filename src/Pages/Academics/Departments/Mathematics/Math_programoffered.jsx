...
const Programmesoffer = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch(
      "https://drc-server.onrender.com/Math_ProgramOffered"
    );
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(
      `https://drc-server.onrender.com/delete_Math_ProgramOffered/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(
          `https://drc-server.onrender.com/Math_ProgramOffered_add`,
          formData,
          {
            headers: {
