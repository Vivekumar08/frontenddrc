...
import Eco_fac_data from "./Eco_fac_data";
import Eco_fac_data_sup from "./Eco_fac_data_sup";
...
const Eco_faculty = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/eco_faculty");
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_eco_faculty/${id}`, {
      method: "POST",
    });
  ...
  const handleSubmit_cv = async (id, pdf) => {
    ...
    try {
      ...
      if (pdf) {
        // setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/eco_faculty_cv_upload/${id}`,
          {
            file: pdf,
          },
          {
      ...
      if (link.trim() !== "" && caption.trim() !== "") {
        if (imag) {
          ...
          const data = await axios.post(
            `https://drc-server.onrender.com/eco_faculty_file_upload`,
            // { method: "POST" },
