...
import History from "../../../../Components/DepartSIde/History";
import Hist_fac_data from "./Hist_fac_data";
import Hist_fac_data_sup from "./Hist_fac_data_sup";
...
const Hist_faculty = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Hist_faculty");
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_Hist_faculty/${id}`, {
      method: "POST",
    });
  ...
  const handleSubmit_cv = async (id, pdf) => {
    ...
    try {
      ...
      if (pdf) {
        // setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Hist_faculty_cv_upload/${id}`,
          {
            file: pdf,
          },
          {
      ...
      if (link.trim() !== "" && caption.trim() !== "") {
        if (imag) {
          ...
          const data = await axios.post(
            `https://drc-server.onrender.com/Hist_faculty_file_upload`,
            // { method: "POST" },
