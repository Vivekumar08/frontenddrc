...
import Sidebar from "../../../../Components/DepartSIde/Chemistry";
...
function Awards() {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Chem_Awards");
  ...
  const handleSubmit_img = async (id, file) => {
    try {
      if (file) {
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Chem_Awards_img_upload/${id}`,
          { file: file },
          {
            headers: {
  ...
  const del_para = async (id, pid, type) => {
    try {
      ...
      console.log(id, arr);
      const response = await fetch(`https://drc-server.onrender.com/delete_Chem_Awards_para/${id}`, {
        method: "POST",
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_Chem_Awards/${id}`, {
      method: "POST",
    });
    await response.json();
    if (response.status === 200) {
  ...
  const delete_file = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `https://drc-server.onrender.com/delete_pdf_link_Chem_Awards_fac/${id}`,
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    console.log(file_path1);
    await axios.post(
      `https://drc-server.onrender.com/delete_img_Chem_Awards_fac/${id}`,
      { file_path1: file_path1, pid: pid },
      {
        method: "POST",
      }
  ...
  const handleSubmit_data = async (id, para) => {
    try {
      if (para !== "") {
        ...
        console.log(arr);
        await fetch(`https://drc-server.onrender.com/Chem_Awards_add_para/${id}`, {
          method: "POST",
          body: JSON.stringify(arr),
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleSubmit_link = async (id, link) => {
    try {
      console.log(link);
      await axios.post(`https://drc-server.onrender.com/Chem_Awards_add_link/${id}`, {
        link: link,
      });
      setCaption("");
      setAuth(true);
  ...
  const handleSubmit_file = async (id, pdf) => {
    ...
    try {
      ...
      if (pdf) {
        await axios.post(
          `https://drc-server.onrender.com/Chem_Awards_file_upload/${id}`,
          {
            file: pdf,
          },
          {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`https://drc-server.onrender.com/Chem_Awards_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
