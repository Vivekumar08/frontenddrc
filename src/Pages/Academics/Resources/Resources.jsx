...
const Resources = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Resource_centre");
  ...
  const handleSubmit_img = async (id,file) => {
    try {
      if (file) {
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Resource_center_img_upload/${id}`,
          { file: file },
          {
            headers: {
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_Resource_centre_fac/${id}`, {
      method: "POST",
    });
    await response.json();
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `https://drc-server.onrender.com/delete_img_Resource_centre_fac/${id}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`https://drc-server.onrender.com/Resource_centre_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
