...
const trainingp = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Training_");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `https://drc-server.onrender.com/delete_academicsTraings/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Academics_Training_add`,
          formData,
          {
            headers: {
        ...
        <div className="w-[1100px]">
          ...
          <div className="main flex-col">
            <table className=" w-96 h-48 ml-3 md:table-fixed md:ml-20 md:w-[500px] md:h-[180px] lg:ml-32 lg:w-[800px] lg:h-[180px] mt-1 ">
              ...
              {data1 &&
                data1.sort(sortOn("link")).map((curElem,i) => {
