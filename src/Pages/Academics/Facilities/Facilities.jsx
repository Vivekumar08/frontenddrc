...
const Facilities = () => {
  ...
  const handleSubmit_img = async (id,file) => {
    try {
      if (file) {
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Academics_Facilities_img_upload/${id}`,
          { file: file },
          {
            headers: {
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`https://drc-server.onrender.com/delete_Academics_Facilities_fac/${id}`, {
      method: "POST",
    });
    await response.json();
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `https://drc-server.onrender.com/delete_img_Academics_Facilities_fac/${id}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`https://drc-server.onrender.com/Academics_Facilities_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
