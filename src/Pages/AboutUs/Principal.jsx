...
const Principal = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("https://drc-server.onrender.com/Principal_About");
  ...
  const del = async (id, pid, type) => {
    try {
      ...
      const response = await fetch(
        `https://drc-server.onrender.com/delete_Principal_About_data/${id}`,
        {
          method: "POST",
  ...
  const handleSubmit = async (files) => {
    try {
      if (files) {
        ...
        setErrMsg("");
        await axios.post(
          `https://drc-server.onrender.com/Principal_About_add`,
          { file: files },
          {
            headers: {
  ...
  const handleSubmit_data = async (id) => {
    try {
      if (para !== "") {
        ...
        console.log(arr);
        await fetch(`https://drc-server.onrender.com/Principal_About_add_data/${id}`, {
          method: "POST",
          body: JSON.stringify(arr),
          headers: { "Content-Type": "application/json" },
        });
                  ...
                  <>
                    <div className="flex flex-row  items-center p-4">
                      <img
                        ...
                        alt="founder"
                        className="rounded-3xl border-black border-2  md:h-[300px] md:w-[380px] ml-2 md:ml-28 lg:ml-96"
                      />
                      {auth && (
                        <>
                ...
                return (
                  <>
                    <div className="flex flex-row  items-center px-4 pt-1 ">
                      <span className="leading-7 font-medium text-justify text-base md:text-md  ">
                        {elem.para1}
                      </span>
                      {auth && (
                        <>
